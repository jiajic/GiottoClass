...
.subset_giotto <- function(
    ...
    # children themselves are never mutated (cf. unionParquetExprStore's
    # ops-clean substore invariant -- mutating children would break
    # union reassembly downstream).
    ...
    # cell spatial network (per-child slot on gmulti; skip there)
    if (!is_multi) {
    ...
    ## spatial info (per-child slot on gmulti; skip)
    ...
    ## feature info (per-child slot on gmulti; skip)
    ...
    ## (nested by spat_unit / feat_type). Federation getters consume
    ## these to filter per-child returns transparently.
    if (is_multi) {
        if (!is.null(cell_ids)) {
